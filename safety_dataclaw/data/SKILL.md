---
name: safety-dataclaw
description: >
  Export Claude Code, Codex, Gemini CLI, and OpenCode conversation history to TRACED.
  Use when the user asks about exporting conversations, uploading to TRACED,
  configuring safety-dataclaw, reviewing PII/secrets in exports, or managing their dataset.
allowed-tools: Bash(safety-dataclaw *), Bash(pip install safety-dataclaw*), Bash(grep *)
---

<!-- safety-dataclaw-begin -->

# Safety-DataClaw Skill
...
1. Get an API key at https://traced.run/settings
2. Run: `safety-dataclaw auth <API_KEY>`
...
`command -v safety-dataclaw >/dev/null 2>&1 && echo "safety-dataclaw: installed" || echo "NOT INSTALLED -- run: pip install safety-dataclaw"`
