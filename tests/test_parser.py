"""Tests for safety_dataclaw.parser — JSONL parsing and project discovery."""
...
from safety_dataclaw.parser import (
    _build_project_name,
...
class TestDiscoverProjects:
    def _disable_codex(self, tmp_path, monkeypatch):
        monkeypatch.setattr("safety_dataclaw.parser.PROJECTS_DIR", tmp_path / "no-claude-projects")
        monkeypatch.setattr("safety_dataclaw.parser.CODEX_SESSIONS_DIR", tmp_path / "no-codex-sessions")
        monkeypatch.setattr("safety_dataclaw.parser.CODEX_ARCHIVED_DIR", tmp_path / "no-codex-archived")
        monkeypatch.setattr("safety_dataclaw.parser._CODEX_PROJECT_INDEX", {})
        monkeypatch.setattr("safety_dataclaw.parser.GEMINI_DIR", tmp_path / "no-gemini")
        monkeypatch.setattr("safety_dataclaw.parser.OPENCODE_DB_PATH", tmp_path / "no-opencode.db")
        monkeypatch.setattr("safety_dataclaw.parser._OPENCODE_PROJECT_INDEX", {})
    ...
    def test_with_projects(self, tmp_path, monkeypatch, mock_anonymizer):
        ...
        monkeypatch.setattr("safety_dataclaw.parser.PROJECTS_DIR", projects_dir)
    ...
    def test_no_projects_dir(self, tmp_path, monkeypatch):
        self._disable_codex(tmp_path, monkeypatch)
        monkeypatch.setattr("safety_dataclaw.parser.PROJECTS_DIR", tmp_path / "nonexistent")
    ...
    def test_parse_nonexistent_project(self, tmp_path, monkeypatch, mock_anonymizer):
        self._disable_codex(tmp_path, monkeypatch)
        monkeypatch.setattr("safety_dataclaw.parser.PROJECTS_DIR", tmp_path / "projects")
        assert parse_project_sessions("nope", mock_anonymizer) == []

    def test_discover_codex_projects(self, tmp_path, monkeypatch):
        self._disable_codex(tmp_path, monkeypatch)
        projects_dir = tmp_path / "projects"
        monkeypatch.setattr("safety_dataclaw.parser.PROJECTS_DIR", projects_dir / "nonexistent")
        ...
        monkeypatch.setattr("safety_dataclaw.parser.CODEX_SESSIONS_DIR", tmp_path / "codex-sessions")
        monkeypatch.setattr("safety_dataclaw.parser.CODEX_ARCHIVED_DIR", tmp_path / "codex-archived")
        monkeypatch.setattr("safety_dataclaw.parser._CODEX_PROJECT_INDEX", {})
    ...
    def test_parse_codex_project_sessions(self, tmp_path, monkeypatch, mock_anonymizer):
        monkeypatch.setattr("safety_dataclaw.parser.PROJECTS_DIR", tmp_path / "projects" / "nonexistent")
        monkeypatch.setattr("safety_dataclaw.parser._CODEX_PROJECT_INDEX", {})
    ...
    def test_discover_opencode_projects(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("safety_dataclaw.parser.OPENCODE_DB_PATH", db_path)
        monkeypatch.setattr("safety_dataclaw.parser._OPENCODE_PROJECT_INDEX", {})
